...
// BigQuery uses RE2 natively, so most patterns pass through unchanged.
...
func convertRE2ToBigQuery(re2Pattern string) (string, bool, error) {
	...
	// 3. Detect unsupported features (lookahead/lookbehind not in RE2 anyway)
	...
	// Process pattern: BigQuery uses RE2 natively, so minimal conversion needed
	caseInsensitive := false
	pattern := re2Pattern

	// Handle (?i) flag - BigQuery REGEXP_CONTAINS embeds the flag in the pattern
	...
	if strings.Contains(pattern, "(?m") || strings.Contains(pattern, "(?s") || strings.Contains(pattern, "(?-") {
		return "", false, errors.New("inline flags other than (?i) are not supported in BigQuery regex")
	}

	// Convert non-capturing groups (?:...) to regular groups (...)
	pattern = strings.ReplaceAll(pattern, "(?:", "(")

	// BigQuery RE2 supports \d, \w, \s, \b natively - no conversion needed
