...
// DuckDB uses RE2 natively, so most patterns pass through unchanged.
...
func convertRE2ToDuckDB(re2Pattern string) (string, bool, error) {
	...
	// Process pattern: DuckDB uses RE2 natively, so minimal conversion needed
	...
	if strings.Contains(pattern, "(?m") || strings.Contains(pattern, "(?s") || strings.Contains(pattern, "(?-") {
		return "", false, errors.New("inline flags other than (?i) are not supported in DuckDB regex")
	}

	// Convert non-capturing groups (?:...) to regular groups (...)
	pattern = strings.ReplaceAll(pattern, "(?:", "(")

	// DuckDB RE2 supports \d, \w, \s, \b natively - no conversion needed
